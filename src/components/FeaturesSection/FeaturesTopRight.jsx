...
const ServicesList = () => {
  ...
  const serviceKeys = ["screening", "sessions", "plans", "support"];

  const services = serviceKeys.map((key) => ({
    title: t(`features.services.${key}.title`),
    description: t(`features.services.${key}.desc`),
  }));
    ...
    >
      ...
      <ol className="list-decimal list-inside space-y-5">
        {services.map((service, index) => (
          <li
            key={index}
            className={`text-cyan pb-3 ${
              index < services.length - 1 ? "border-b border-darkCyan/30" : ""
            } 
          ...
          >
            ...
            <br />
            <span className="text-sm block mt-1">{service.description}</span>
          </li>
        ))}
      </ol>
    </div>
