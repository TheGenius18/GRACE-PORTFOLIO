...
const Face = ({ emotionLevel, scaleLevel, blushOpacity }) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!faceRef.current || !eyeLeftRef.current || !eyeRightRef.current)
        return;
      ...
      const headRotationX = Math.min(
        Math.max(cursorY / (faceRect.height * 2), 0.1)
      );
      const headRotationY = Math.min(
        Math.max(cursorX / (faceRect.width * 3), -0.2),
        0.2
      );
...
const Eyebrows = ({ expression }) => (
  <div className="absolute top-[25%] w-full flex justify-center space-x-16">
    <motion.div
      ...
      animate={{
        y: expression.includes("happy")
          ? 2
          : expression.includes("sad")
          ? -1
          : 0,
        rotate: expression.includes("happy")
          ? -4
          : expression.includes("sad")
          ? -15
          : 0,
    ...
    >
      <div
        className="absolute inset-0 bg-[#371b07] rounded-full"
    ...
    >
      <div
        className="absolute inset-0 bg-[#2d1606] rounded-full"
        style={{ clipPath: "ellipse(50% 30% at 50% 50%)" }}
      />
    </motion.div>
  </div>
...
const Eye = ({
  ...
  const emotionMovement =
    emotionLevel > 70 ? Math.sin(Date.now() / 300) * 0.5 : 0;
  ...
  <>
    <motion.div
      ...
      style={{
        ...
        background:
          "radial-gradient(circle, #ff052e 30%, rgba(255,5,46,0.9) 100%)",
      }}
      animate={{ scale: emotionLevel > 70 ? [1, 1.1, 1] : 1 }}
    ...
    <motion.div
      ...
      style={{
        ...
        background:
          "radial-gradient(circle, #ff052e 30%, rgba(255,5,46,1) 100%)",
      }}
      animate={{ scale: emotionLevel > 70 ? [1, 1.1, 1] : 1 }}
      transition={{ duration: 2, repeat: Infinity, repeatType: "mirror" }}
    />
